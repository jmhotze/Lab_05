...
void printMonthlyrep(double sales[]);
...
void printMovingAverages(double sales[]) {
    ...
    for (int i = 0; i < MONTHS - 5; i++) {
        ...
        printf("%s - %s $%.2lf\n", getMonth(i), getMonth(i+5), sum / 6.0);
    }
    printf("\n");
}
...
void printSalesHighToLow(double sales[]) {
    ...
    printf("Month Sales\n");
    for (int i = 0; i < MONTHS; i++) {
        printf("%-10s $%.2lf\n", getMonth(sortedSales[i].index), sortedSales[i].value);
    }
    printf("\n");
}
